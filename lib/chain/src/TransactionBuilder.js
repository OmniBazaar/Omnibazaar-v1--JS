...
class TransactionBuilder {
        ...
        return new Promise((resolve, reject)=> {
            ...
            resolve(Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then((r) => {
                ...
                if( this.expiration === 0 )
                    this.expiration = TransactionBuilder.base_expiration_sec() + ChainConfig.expire_in_secs
    ...
    */
    ...
    get_type_operation(name, operation) {
        ...
        if (name === 'proposal_create') {
            ...
            */
            ...
            operation.expiration_time || (operation.expiration_time = (TransactionBuilder.base_expiration_sec() + ChainConfig.expire_in_secs_proposal));
            if (requiresReview) {
    ...
    set_expire_seconds(sec){
        if (this.tr_buffer) { throw new Error("already finalized"); }
        return this.expiration = TransactionBuilder.base_expiration_sec() + sec;
    }
    ...
    set_required_fees(asset_id){
        ...
        let promises = [
            ...
            Apis.instance().db_api().exec("get_required_omnibazaar_fees", [operations])
        ];
        ...
        let fees, obFees, coreFees, asset;
        return Promise.all(promises).then( (results) => {

            [fees, obFees, coreFees, asset] = results;
        ...
        .then((dynamicObject) => {
            ...
            let set_fee = operation => {
                ...
                operation.ob_fee = obFees[asset_index];
                asset_index++;
    ...
    static fetch_base_expiration_sec() {
        return Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then((r) => {
            head_block_time_string = r[0].time;
            return TransactionBuilder.base_expiration_sec();
        });
    }
}
